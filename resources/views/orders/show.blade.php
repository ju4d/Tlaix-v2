        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    @endphp
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $statusClasses[$order->status] ?? 'bg-gray-100 text-gray-800' }}">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tfoot class="bg-gray-50">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-900">
                                ${{ number_format($order->total * 0.16, 2) }}
                            </td>
                        </tr>
                        <tr>
            ...
            @if($order->status == 'pending')
                <div class="flex space-x-3">
                    <form method="POST" action="{{ url('/orders/' . $order->id . '/receive') }}" class="inline">
                        @csrf
                        <button type="submit" onclick="return confirm('¿Confirmar que se recibió este pedido?')"
                    ...
                    <form method="POST" action="{{ route('orders.cancel', $order->id) }}" class="inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit" onclick="return confirm('¿Confirmar cancelación de este pedido?')"
